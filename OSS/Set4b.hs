...
slStart = (0.0, 0)
slHelper x start = (fst start + x, snd start + 1)
-- x : xs의 원소들
-- start : foldr tree의 오른쪽 맨 아래
------------------------------------------------------------------------------
...
largestHelper x [] = x:[]
largestHelper x start
    | x > (head start) = [x]
    | x == (head start) = x:start
    | x < (head start) = start
...
lastHelper x Nothing = head [Just x]
lastHelper x start = start

-- start : foldr tree의 오른쪽 node
-- x : foldr tree의 왼쪽 node