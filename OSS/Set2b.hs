...
leftpad s n = if length s >= n -- leftpad "xxxxx" 3 ==> "xxxxx
    then s
    else leftpad2 s (n - length s)

leftpad2 :: String -> Int -> String
leftpad2 s n = if n == 0
    then s
    else leftpad2(" " ++ s) (n-1) -- n과 s길이의 차이 만큼 " " 추가
------------------------------------------------------------------------------
...
countdown n = "Ready! " ++ countdown2(n)

countdown2 :: Integer -> String
countdown2 n = if n == 1
    then show n ++ "... Liftoff!"
    else show n ++ "... "++ countdown2(n-1)
...
oddMin :: Integer -> Integer -> Integer
oddMin n k = if n `mod` k == 0
    then k
    else oddMin n (k+1)
------------------------------------------------------------------------------
...
isPrime 0 = False
isPrime 1 = False
isPrime n = if n == smallestDivisor n
    then True
    else False
------------------------------------------------------------------------------
